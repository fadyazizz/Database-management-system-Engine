...
public class BPlusTree<K extends Comparable<? super K>, V> implements Serializable{
	...
	static BPlusTree.Node temproot;
...
public BPlusTree() {
	...
	public BPlusTree(int branchingFactor,String TableName,String ColName) throws IOException {
...
//	}
	...
	**/
	static void Delete(String path) {
	 ...
	 public void checker(String tableName,String indexName) {
		 try {
			...
			Table tb=(Table)s;
				...
				createBTreeIndex(tableName, indexName);
	 ...
	 public static void createBTreeIndex(String strTableName, String strColName) throws Exception {
			...
			if (colType.equals("java.lang.Boolean") || colType.equals("java.awt.Polygon")) {
				System.out.println("cantt create index on Boolean or polygon");
				return;
			}
...
//	}
	public void delete(K key) throws Exception {
		...
		//this.deleteWrongPages();
		if(root.keys.size()==0) {
			DBApp.serialize(temproot.path, temproot);
		}
		else {
			DBApp.serialize(root.path, root);
		}
		//DBApp.serialize(root.path, this.root);
		DBApp.serialize(this.TableName+"_"+this.ColName+"Index.txt", this);
	}
	...
	public String toString(){
	 ...
	 class InternalNode extends Node implements Serializable {
		...
		void deleteValue(K key) throws Exception {
			...
			if (child.isUnderflow()) {
				...
				if (left.isOverflow()) {
					...
					if(sibling instanceof BPlusTree.LeafNode) {
				...
				this.insertChild(left, left.keys.get(0));
				if  (  this.path.equals(root.path) &&  this.keyNumber() == 0) {
					///left.Delete();
					System.out.println("leftkeys "+left.keys);
					//allLeafs.remove(left.path);
					
					left.path="index_On_gamed_fadyroot.txt";
					root = left;
					temproot=left;
					System.out.println(key);
					DBApp.serialize(root.path, temproot);
				//	DBApp.serialize("allLeafs"+ColName+"_"+TableName+".txt", allLeafs);
					return;
				}
		...
		void insertValue(K key, V value) throws Exception {
			...
			if(childNode instanceof BPlusTree.InternalNode) {
		...
		private void insertChild(BPlusTree<K, V>.Node childNode,K key) {
			int index=0;
	 ...
	 class LeafNode extends Node implements Serializable  {
		...
		void insertValue(K key, V value) throws Exception {
			...
			if(root instanceof BPlusTree.LeafNode  ) {
				if(!allNodes.contains(root.path)) {
					allNodes.add(root.path);
				}
